...
impl TransactionLog {
    ...
    async fn save(&mut self, db: &PgPool) -> Result<(), String> {
        ...
        sqlx::query(
            r#"INSERT INTO transactions_log (wallet_address, from_token, to_token, amount_from, amount_to)
        VALUES ($1, $2, $3, $4, $5)"#
        )
...
pub async fn log_transaction(
    ...
    let mut tx = TransactionLog::new(wallet_address, from_token, to_token, amount_from, amount_to);
    tx.save(db).await?;
    Ok(tx)
}
